...
import { LogIn, LogOut, User as UserIcon, Activity, Sparkles, FileEdit, Key, X } from "lucide-react";
...
export function Navbar({ activeTab, setActiveTab }: NavbarProps) {
  ...
  return (
    <>
      <nav className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 sticky top-0 z-40">
        ...
        <div className="flex items-center space-x-4">
          <button 
            ...
            className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-500 hover:bg-gray-100 hover:text-indigo-600 rounded-lg transition-colors border border-transparent hover:border-gray-200"
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <p className="mt-2 text-xs text-gray-500">
                    Ваш ключ сохраняется только в локальном хранилище вашего браузера (localStorage) и никуда не передается, кроме API RouterAI.
                  </p>
                </div>
                <button
                  onClick={saveApiKey}
                  className="w-full bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors shadow-sm"
                >
                  Сохранить ключ
                </button>
              </div>
